                    ...
                    if let document = document, document.exists {
                        if let username = document.data()?["Username"] as? String,
                           let photoURL = document.data()?["ProfilePictureURL"] as? String{
                            let highlightInfo = HighlightInfo(uid: userDocumentID, postID: pdocument.documentID, imageName: imagePath, profileImageName: photoURL, username: username, highlightTitle: caption, likedUsers: likedUsersArray)
