...
struct RecentMessageRow: View {
  ...
  var body: some View {
    NavigationLink(destination: ChatView(chatUser: ChatUser(uid: recentMessage.toId == Auth.auth().currentUser?.uid ? recentMessage.fromId : recentMessage.toId, email: recentMessage.email, profileImageUrl: photoURL))) {
      HStack {
        Button {
